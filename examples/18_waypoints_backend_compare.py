...
DOFS = 7
CONTROL_CYCLE = 0.01
WAYPOINT_SPACING = 0.2  # rad; paper reports Ruckig Pro with 0.1 and 0.2 rad sampling.


def _dense_lemniscate_path(n_samples=2001):
    """Paper-like seven-joint geometric reference path.

    The paper evaluates 7-DoF KUKA iiwa paths shaped like simple geometric
    curves. The original sampled datasets are not in this repo, so this builds
    a deterministic joint-space lemniscate with all joints moving smoothly.
    """
...
def _resample_polyline(poly, spacing):
...
def make_input():
    ref_poly = _resample_polyline(_dense_lemniscate_path(), WAYPOINT_SPACING)
    ...
    inp.intermediate_positions = [
        p.tolist() for p in ref_poly[1:-1]
    ]
    ...
    # KUKA iiwa limits from Table I of Kiemel & Kroeger 2024.
    inp.max_velocity = [1.71, 1.71, 1.74, 2.27, 2.44, 3.14, 3.14]
    inp.max_acceleration = [15.0, 7.5, 10.0, 12.5, 15.0, 20.0, 20.0]
...
def run_backend(backend, label):
    ...
    out = OutputParameter(DOFS, max_waypoints)
...
def resample_polyline_uniform_arc(poly, n_samples=1000, max_arc=None):
    """Resample an explicit polyline at equal path-length values."""
    diffs = np.diff(poly, axis=0)
    chord = np.linalg.norm(diffs, axis=1)
    arc = np.concatenate([[0.0], np.cumsum(chord)])
    total_arc = arc[-1]
    if total_arc < 1e-12:
        return poly, total_arc

    sample_arc = total_arc if max_arc is None else min(max_arc, total_arc)
    s_uniform = np.linspace(0.0, sample_arc, n_samples)
    return np.column_stack([
        np.interp(s_uniform, arc, poly[:, d])
        for d in range(poly.shape[1])
    ]), total_arc
...
def total_polyline_length(poly):
...
def compare_paths(runs, ref_poly, n_samples=2000):
    ...
    print("\nPaper-style same-u deviation:")
    print("(dense generated/reference samples compared at the same path length)")
    ref_same_cache = {}
    ref_len = total_polyline_length(ref_poly)
    for run_key, (label, out_list) in runs.items():
        generated = stack(out_list, "new_position")
        _, gen_len = resample_polyline_uniform_arc(generated, n_samples)
        common_len = min(gen_len, ref_len)
        if common_len not in ref_same_cache:
            ref_same_cache[common_len], _ = resample_polyline_uniform_arc(ref_poly, n_samples, common_len)
        gen_same, _ = resample_polyline_uniform_arc(generated, n_samples, common_len)
        same_u_dev = np.linalg.norm(gen_same - ref_same_cache[common_len], axis=1)
        print(f"  {label:6s}  generated_length={gen_len:.6f}  common_u={common_len:.6f}")
        report_deviation(label, same_u_dev)
