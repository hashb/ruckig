...
#if defined RUCKIG_WITH_WAYPOINTS
    //! Set the backend for calculating trajectories with intermediate waypoints (only backends enabled at build time can be used)
    void set_waypoints_backend(WaypointsBackend backend) {
        calculator.waypoints_backend = backend;
        current_input_initialized = false;
    }

    //! Get the backend for calculating trajectories with intermediate waypoints
